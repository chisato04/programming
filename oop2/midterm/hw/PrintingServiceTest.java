...
public class PrintingServiceTest {
    ...
    private Checkbox chkStar, chkText, chkLogo;
    ...
    public PrintingServiceTest() {
        ...
        med = new JRadioButton("MEDIUM    P250");
        med.setBounds(30, 120, 120, 20);

        lg = new JRadioButton("LARGE    P350");
        lg.setBounds(30, 150, 120, 20);
        ...
        chkText = new Checkbox("TEXT      P50");
        chkLogo = new Checkbox("LOGO      P35");
        chkStar.setBounds(225, 90, 120, 35);
        chkText.setBounds(225, 120, 120, 40);
        chkLogo.setBounds(225, 150, 120, 40);
    ...
    public void showResult() {
        ...
        chkText.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                String item = (e.getStateChange() == 1 ? "true" : "false");
                if (item.equals("true")) {

                    ProcessList.add("Text  P50");
                    Total += textPrice;

                } else {

                    ProcessList.remove("Text  P50");
                    Total -= textPrice;
                }
            }
        });

        chkLogo.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                String item = (e.getStateChange() == 1 ? "true" : "false");
                if (item.equals("true")) {

                    ProcessList.add("Logo  P50");
                    Total += logoPrice;

                } else {

                    ProcessList.remove("Logo  P50");
                    Total -= logoPrice;
                }
            }
        });
        ...
        med.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {

                    ProcessList.add("Medium  P250");
                    Total += medPrice;

                } else if (e.getStateChange() == ItemEvent.DESELECTED) {

                    ProcessList.remove("Medium  P250");
                    Total -= medPrice;
                }
            }
        });

        lg.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {

                    ProcessList.add("Large  P350");
                    Total += lgPrice;
                } else if (e.getStateChange() == ItemEvent.DESELECTED) {

                    ProcessList.remove("Large  P350");
                    Total -= lgPrice;
                }
            }
        });
        ...
        cancelBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                chkText.setState(false);
                chkLogo.setState(false);
                Total = 0;
                cash.setText("");
                change.setText("");
    ...
    public static void main(String[] args) {
        PrintingServiceTest ps = new PrintingServiceTest();
        ps.showResult();
    }
}
//fixed text